...
void createInstance(State_t *state)
{
    ...
    LOG_IF_ERROR(vkCreateInstance(&createInfo, state->allocator, &state->instance),
...
void selectPhysicalDevice(State_t *state)
{
    ...
    LOG_IF_ERROR(vkEnumeratePhysicalDevices(state->instance, &count, NULL),
    ...
    if (count > 0)
    {
        ...
        LOG_IF_ERROR(vkEnumeratePhysicalDevices(state->instance, &count, physicalDevices),
    ...
    else
    {
        ...
        LOG_IF_ERROR(true, "No Vulkan-supported physical devices found!")
    }
}
...
void createSurface(State_t *state)
{
    ...
    LOG_IF_ERROR(glfwCreateWindowSurface(state->instance, state->window, state->allocator, &state->surface),
...
void selectQueueFamily(State_t *state)
{
    ...
    LOG_IF_ERROR(state->queueFamily == UINT32_MAX,
...
void createDevice(State_t *state)
{
    ...
    LOG_IF_ERROR(vkCreateDevice(state->physicalDevice, &createInfo, state->allocator, &state->device),
...
void freeSwapchainImages(State_t *state)
{
    ...
    {
        for (uint32_t i = 0U; i < state->swapchainImageCount; i++)
        {
...
void createSwapchain(State_t *state)
{
    ...
    LOG_IF_ERROR(vkGetPhysicalDeviceSurfaceFormatsKHR(state->physicalDevice, state->surface, &formatCount, NULL),
    ...
    LOG_IF_ERROR(vkGetPhysicalDeviceSurfaceFormatsKHR(state->physicalDevice, state->surface, &formatCount, formats),
    ...
    LOG_IF_ERROR(vkGetPhysicalDeviceSurfacePresentModesKHR(state->physicalDevice, state->surface, &presentModeCount, NULL),
    ...
    LOG_IF_ERROR(vkGetPhysicalDeviceSurfacePresentModesKHR(state->physicalDevice, state->surface, &presentModeCount, presentModes),
                 "Failed to query physical device surface presentation modes.")
    ...
    VkSwapchainCreateInfoKHR createInfo = {
        ...
        .pQueueFamilyIndices = &state->queueFamily, // Skip making an array for this since only using 1 queue family
    ...
    LOG_IF_ERROR(vkCreateSwapchainKHR(state->device, &createInfo, state->allocator, &swapchain),
    ...
    // null so that we just get the number of formats
    LOG_IF_ERROR(vkGetSwapchainImagesKHR(state->device, state->swapchain, &state->swapchainImageCount, NULL),
                 "Failed to query the number of images in the swapchain.");
    logger(LOG_INFO, "The swapchain will contain a buffer of %d images.", state->swapchainImageCount);
    state->swapchainImages = malloc(sizeof(VkImage) * state->swapchainImageCount);
    LOG_IF_ERROR(state->swapchainImages == NULL, "Unable to allocate memory for swapchain images.")
    LOG_IF_ERROR(vkGetSwapchainImagesKHR(state->device, state->swapchain, &state->swapchainImageCount, state->swapchainImages),
                 "Failed to get the images in the swapchain.")
    state->swapchainImageViews = malloc(sizeof(VkImageView) * state->swapchainImageCount);
    LOG_IF_ERROR(state->swapchainImageViews == NULL, "Unable to allocate memory for swapchain image views.")
    ...
    for (uint32_t i = 0U; i < state->swapchainImageCount; i++)
    {
        ...
        LOG_IF_ERROR(vkCreateImageView(state->device, &createInfo, state->allocator, &state->swapchainImageViews[i]),
